...
class Inicio:
    def main():
        ...
        nombre_buscado = 'Pedro'
        cursor.execute('SELECT * FROM Alumnos WHERE nombre = ?', (nombre_buscado,))
        alumno_encontrado = cursor

        print(alumno_encontrado)

        # Mostrar los datos del alumno por consola
        if alumno_encontrado:
            print(f'Datos del alumno con nombre "{nombre_buscado}":')
            print('IDr:', alumno_encontrado[0])
    ...
    def ventana():
        ...
        tabla = ttk.Treeview(ventana, columns=("Nombre", "Apellido", "Edad"),show="headings")
